...
void WeinerST::insertString(bool text_node) {
...
void WeinerST::extend(int i, bool text_node) {
  ...
  while (node1 != root_ && !node1->checkIndicator(c)) {
    ...
    if (!text_node) {
      tree_mutations_.push(Mutation(AddIndicator, node1));
    }
  ...
  if (!node1->checkIndicator(c)) {
    ...
    // node1 is the first node on the path up from prev_leaf_ such with indicator c.
    ...
    if (!node2->checkLink(c)) {
      ...
      head_node = breakEdge(child_node, c, t);
    }
    else {
      ...
      if (path_length == 0) {
        ...
        // head(i) ends in the middle of the edge
        head_node = breakEdge(child_node, c2, path_length);
      }
    }
  ...
  WeinerLeaf* new_leaf = new WeinerLeaf(head_node,
                                        ...
                                        i, s.length() - i,
                                        i);
  head_node->addChild(s[tail_start], new_leaf);
  prev_leaf_ = new_leaf;
}
...
WeinerInode* WeinerST::breakEdge(WeinerNode* const child_node,
  ...
  WeinerInode* new_node = new WeinerInode(parent_node,
                                          ...
                                          child_path_label.start_, child_path_label.length_ - child_edge_label.length_ + edge_label_prefix_length); // * Tree mutation *
  ...
  if (child_node->isLeaf()) {
    ...
    new_node->addIndicator(child_node->isTextNode() ? text_string_[j] : pattern_string_[j]);
  }
  else {
...
void WeinerST::printTreeTraverse(WeinerNode* const node, int indent) {
  ...
  if (node->isLeaf()) {
    ...
    std::printf((line_prefix + getLabelString(inode->getEdgeLabel()) + " -> Inode (" + (inode->isTextNode() ? "T" : "P") + ") (" + getLabelString(inode->getPathLabel())  + ") \n").c_str());
    std::map<char, WeinerNode*> children = inode->getChildren();
    for (std::map<char, WeinerNode*>::iterator iter = children.begin(); iter != children.end(); iter++) {
